...
def write_in_script(sigma, numParticleTypes, PatchRange, PatchStrength, IsotropicAttrRange, IsotropicAttrStrength, real,
                    RunSteps, dumpevery, filePattern, configName, ResultsFolder, extForce, BondStrength, AngleStrength, NAngleSlices):
    seed = 100 + real
    ...
    clusterevery=100
    assert dumpevery%clusterevery==0, "Error: dumpevery must be a multiple of clusterevery."
    ...
    f.write("read_data              {:s}  extra/bond/per/atom 10  extra/angle/per/atom 1000  extra/special/per/atom 50 \n\n".format(configName))
    ...
    f.write("comm_modify            mode single cutoff 2.5 \n")
    ...
    f.write("\nbond_style             harmonic \n")
    f.write("bond_coeff             1 {:.1f} 1\n".format(BondStrength))

    f.write("\nangle_style            harmonic \n")
    OneSliceAngle=180/(NAngleSlices-1)
    for i in range(0,NAngleSlices):
        f.write("angle_coeff        {:d} {:.1f} {:.1f} \n".format(i+1, AngleStrength, OneSliceAngle*i))
    ...
    f.write("run                    0 \n")
    f.write("compute                cCluCentral Central cluster/atom {:f} \n".format(sigma+IsotropicAttrRange))
    f.write("compute                cCluReduce Central reduce/chunk cMol min c_cCluCentral \n")
    #f.write("compute                cClustersChunk Central chunk/atom c_cClustersCentral \n")
    f.write("compute                cCluSpread all chunk/spread/atom cMol c_cCluReduce \n")
    #f.write("fix                fClu all property/atom c_cCluSpread \n")
    f.write("variable                vClu atom c_cCluSpread \n")
    f.write("dump                   dClu all custom {:d} {:s}/ClustersLammps_{:s}.dat id c_cCluCentral c_cCluSpread v_vClu \n".format(dumpevery, ResultsFolder, filePattern))
    ...
    f.write("\nfix                    fLANG all langevin {:.3f} {:.3f} {:.3f} {:d} \n".format(Temperature, Temperature,
                                                                                            LangevinDamping, seed))
    #f.write("fix                    fRigidNVE all rigid/nve molecule\n")
    f.write("fix                    fRigidNVE all rigid/nve  custom v_vClu  reinit yes\n")
    ...
    # for i in range(0, NAngleSlices):
    #       f.write("fix                    fbonds{:d} Central bond/create/angle 10 1 1 {:.3f} 1 atype {:d} aconstrain {:d} {:d} prob 0.5 {:d}\n".format(
    #           i+1, sigma+0.05, i+1, int(max([0,(i-0.5)*OneSliceAngle])), int(min([180,(i+0.5)*OneSliceAngle])), seed ) )
    #f.write("fix                    fbondsbreak Central bond/break 1 1 {:.3f} \n\n".format(sigma+IsotropicAttrRange))
    ...
    if np.isclose(extForce, 0, atol=1e-6, rtol=0)==False:
        ...
        f.write("\nvariable               fCentralModulus atom sqrt(v_fxCentral^2+v_fyCentral^2)\n")
    ...
    f.write("fix                    fAngMomTotAvgt all ave/time 10 10 {:d} v_vAngMomTot \n".format(min(dumpevery,clusterevery)))
    f.write("variable               vTorqueTot equal torque(all,z) \n")
    f.write("fix                    fTorqueTotAvgt all ave/time 10 10 {:d} v_vTorqueTot \n".format(min(dumpevery,clusterevery)))
    ...
    f.write("fix                    fAngMomAvgmAvgt all ave/time 10 10 {:d} c_cAngMomAvgm \n".format(min(dumpevery,clusterevery)))
    ...
    f.write("fix                    fTorqueAvgmAvgt all ave/time 10 10 {:d} c_cTorqueAvgm \n".format(min(dumpevery,clusterevery)))
    ...
    f.write("fix                    fOmegaAvgmAvgt all ave/time 10 10 {:d} c_cOmegaAvgm \n".format(min(dumpevery,clusterevery)))
    ...
    if np.isclose(extForce, 0, atol=1e-6, rtol=0)==False:
        ...
        f.write("fix                    fAddforceCheck0 all ave/time 10 10 {:d} v_vAddforceCheck0 \n\n".format(min(dumpevery,clusterevery)))
    ...
    f.write("compute			cBonds all property/local batom1 batom2 btype\n")
    if dumpevery > 0:
        f.write("dump 			dBonds all local {:d} {:s}/Bonds_{:s}.dat index c_cBonds[1] c_cBonds[2] c_cBonds[3]\n".format(
                dumpevery, ResultsFolder, filePattern))
    f.write("dump_modify            dBonds  every v_dumpts first yes\n")

    ########
    # RUN
    ########

    # for i in range(1,int(RunSteps/dumpevery)):
    #     f.write("\nrun                    {:d} \n".format(dumpevery))   # 'delete_bonds Central multi remove special'

    f.write("\nrun                    {:d}  post no  every {:d} 'unfix  fRigidNVE'  'unfix  fEnforce2d'  'fix  fRigidNVE all rigid/nve  custom v_vClu  reinit yes'  'fix  fEnforce2d all enforce2d'  'delete_bonds  Central multi remove ' \n".format(RunSteps, clusterevery))   # 'delete_bonds Central multi remove special'
...
if __name__ == "__main__":
    ...
    parser.add_argument('-extTorque', '--extTorque', '-eT', dest='extTorque', action='store', type=float,
                        default=extTorque, help='Applied external torque.')
    parser.add_argument('-Kb', '--BondStrength', dest='BondStrength', action='store', type=float,
                        default=BondStrength, help='Bond Strength. Default {}.'.format(BondStrength))
    parser.add_argument('-Ka', '--AngleStrength', dest='AngleStrength', action='store', type=float,
                        default=AngleStrength, help='Angle Strength. Default {}.'.format(AngleStrength))
    ...
    BondStrength = args.BondStrength
    AngleStrength = args.AngleStrength
    ...
    filePattern = "qA{:d}_dp{:.2f}_dens{:.2f}_eT{:.2f}_nA{:d}_rp{:.2f}_ra{:.2f}_ep{:.1f}_ea{:.1f}_Kb{:.0f}_Ka{:.0f}_T{:.1f}_{:d}".format(
        q_A, PatchRadialDistance, density_A, extTorque, num_A, PatchRange, IsotropicAttrRange, PatchStrength, IsotropicAttrStrength, BondStrength, AngleStrength, Temperature, real)
    ...
    inputscriptfile = write_in_script(sigma, system.numTypes, PatchRange, PatchStrength, IsotropicAttrRange,
                                      ...
                                      ResultsFolder, extForce, BondStrength, AngleStrength,NAngleSlices)
