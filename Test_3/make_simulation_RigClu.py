...
def write_in_script(sigma, numParticleTypes, PatchRange, PatchStrength, IsotropicAttrRange, IsotropicAttrStrength, real,
                    RunSteps, dumpevery, filePattern, configName, ResultsFolder, extForce, AngleStrength, NAngleSlices):
    seed = 100 + real
    ...
    #f.write("fix                    fdummy all rigid/nve molecule \n")
    #f.write("run                    1\n")
    ...
    #f.write("dump                   ddummy all custom 1 {:s}/RigidBodies-t0_{:s}.xyz id type mol v_vClu \n".format(ResultsFolder, filePattern))
    ...
    f.write("fix                    fRigidNVE all rigid/nve custom v_vClu \n")
    ...
    for i in range(0, NAngleSlices):
           f.write("fix                    fbonds{:d} Central bond/create/angle 2 1 1 {:.3f} 1 atype {:d} aconstrain {:d} {:d} prob 0.1 {:d}\n\n".format(
...
if __name__ == "__main__":
    ...
    ResultsFolder = "Results_RigClu_qA{:d}_dp{:.2f}_dens{:.3f}_eT{:.2f}".format(q_A, PatchRadialDistance, density_A, extTorque)
    ...
    inputscriptfile = write_in_script(sigma, system.numTypes, PatchRange, PatchStrength, IsotropicAttrRange,
                                      ...
                                      ResultsFolder, extForce, AngleStrength,NAngleSlices)
