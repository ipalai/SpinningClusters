...
def write_in_script(sigma, numParticleTypes, PatchRange, PatchStrength, IsotropicAttrRange, IsotropicAttrStrength, real,
                    RunSteps, dumpevery, filePattern, configName, ResultsFolder, extForce, clusterevery, Lbox):
    seed = 100 + real
    ...
    f.write("compute                cImages all property/atom ix iy iz \n")
    ...
    ##################
    # TIME INTEGRATION (except walls)
    ##################

    f.write("\nfix                    fLANG all langevin {:.3f} {:.3f} {:.3f} {:d} \n".format(Temperature, Temperature,
                                                                                            LangevinDamping, seed))

    f.write("fix                    fRigidNVE all rigid  custom v_vClu  reinit yes\n")
    ...
    f.write("region               Walls block  EDGE EDGE EDGE EDGE INF INF \n")
    f.write("fix                  fWalls Central wall/region Walls lj126 20.0 {:.6f} {:.6f} \n".format(sigma,sigma*2**(1.0/6.0)))
    ...
    f.write("dump_modify            1Log  sort id  flush yes  every v_dumpts  first yes\n")
    ...
    if np.isclose(extForce, 0, atol=1e-6, rtol=0)==False:
        f.write("dump                   3 CentralAndPatch custom {:d} {:s}/CheckTorque_{:s}.dat id type mol x y c_cCentral[1] c_cCentral[2] c_cPatch[1] c_cPatch[2] c_cxu c_cyu v_fxCentral v_fyCentral v_fxPatch v_fyPatch v_fCentralModulus v_fPatchModulus \n".format(
                dumpevery, ResultsFolder, filePattern))
        f.write("dump_modify            3 sort id flush yes \n\n")

    #f.write("fix                    4 all ave/time 1 1 {:d} f_fRigidNVE[*]  mode vector file {:s}/ClusterProperties_{:s}.dat \n".format(dumpevery, ResultsFolder, filePattern))
    ...
    #f.write("\nrun                    {:d}  post no  every {:d} 'unfix  fRigidNVE'  'unfix  fEnforce2d'  'fix  fRigidNVE all rigid/nve  custom v_vClu  reinit yes'  'fix  fEnforce2d all enforce2d' \n".format(RunSteps, clusterevery))
    f.write("\nrun                    {:d}  post no  every {:d}  'fix  fRigidNVE all rigid  custom v_vClu  reinit yes' \n".format(RunSteps, clusterevery))
...
if __name__ == "__main__":
    ...
    PatchStrength = -10 #-IsotropicAttrStrength
    ...
    parser.add_argument('-clusterevery', '--clusterevery', '-ce', dest='clusterevery', action='store', type=int, default=clusterevery,
                        help='Compute rigid bodies every this many timesteps.')
    ...
    clusterevery = args.clusterevery
    ...
    filePattern = "qA{:d}_dp{:.2f}_dens{:.2f}_eT{:.2f}_nA{:d}_rp{:.2f}_ra{:.2f}_ep{:.1f}_ea{:.1f}_ce{:d}_T{:.1f}_{:d}".format(
        q_A, PatchRadialDistance, density_A, extTorque, num_A, PatchRange, IsotropicAttrRange, PatchStrength, IsotropicAttrStrength, clusterevery, Temperature, real)
